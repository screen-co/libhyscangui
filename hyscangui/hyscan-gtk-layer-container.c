 ...
 *   1.1 определяет, в каком из слоёв хэндл лежит ближе всего к курсору,
 ...
 *   2.1 определяет, в каком из слоёв хэндл лежит ближе всего к курсору,
 ...
 *
 * ## Выделение объектов на слое
 *
 * Класс #HyScanGtkLayerContainer даёт возможность взаимодействия с несколькими
 * объектами слоя через механизм выделения объектов на области. Для этого слой
 * должен имлементировать функции #HyScanGtkLayerInterface.select_mode и
 * #HyScanGtkLayerInterface.select_area.
 *
 * Пользователь может перевести слой в режим выделения, зажав клавишу Ctrl
 * и с помощью мыши выбрать область виджета. Текущий слой с правом ввод (input_owner)
 * сообщит список объектов, которые оказались внутри выбранной области, а также
 * может предоставить один или несколько хэндлов для взаимодейсвтия с выбранной
 * группой объектов.
 *
 * ## Конфигурация внешнего вида слоёв
 *
 * Контейнер, а также каждый из слоёв, могут определять параметры #HyScanParam,
 * для настройки внешнего вида и параметров отображения. Параметры отображения
 * как правило содержат цвета и размеры элементов, выводимых на слоях.
 *
 * Функция hyscan_gtk_layer_container_get_param() формирует общий HyScanParam
 * конфигурации виджета, получая параметры каждого из слоёв через hyscan_gtk_layer_get_param().
 *
 * Использование общего HyScanParam контейнера позволяет установить согласованную
 * цветовую схему всех слоёв и сохранить её в виде отдельного профиля.
 *
 */
...
/* Обрабатывает клик или захват хэндла под указателем мыши. */
static gboolean
 ...
 * @container: указатель на #HyScanGtkLayerContainer
 * @instance: новый владелец хэндла или %NULL
 *
