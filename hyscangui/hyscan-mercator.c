 ...
 *
 * Считая, что Земля имеет форму эллипсоида, класс позволяет переводить
 * географические координаты (широта, долгота) в координаты на плоскости проекции
 * (x, y) и обратно. Соответствующие функции: hyscan_mercator_value_to_geo() и
 * hyscan_mercator_geo_to_value().
 *
 * Парметры эллипсоида можно указать при создании объекта через hyscan_mercator_new().
 *
 * Проекция Меркатора не сохраняет постоянный масштаб на всей плоскости поверхности.
 * Чтобы получить значение масштаба в опредленной точке используйте
 * hyscan_mercator_get_scale().
 *
 * ## Границы проекции
 *
 * Проекция Меркатора не отображает полярные регионы, находящиеся дальше параллели 85°.
 * Границы проекции на плоскости можно получить с помошью hyscan_mercator_get_limits().
 *
 */
...
/* Определяет масштаб проекции в указнной точке @coords. */
static gdouble
