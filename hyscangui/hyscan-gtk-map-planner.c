  ...
  while (g_hash_table_iter_next (&iter, (gpointer *) &key, (gpointer *) &object))
    {
      ...
      if (orig_track->zone_id != NULL)
        zone = g_hash_table_lookup (priv->zones, orig_track->zone_id);
                                  ...
                                  cairo_t                 *cairo)
{
  ...
  GList *link;
  gint vertex_num = 0;
  gboolean hovered, hovered_vertex, highlighted;
  HyScanGtkMapPoint *point;
  ...
  highlighted = (g_strcmp0 (priv->highlight_zone, zone->id) == 0 && priv->highlight_vertex >= 0);
  ...
  for (link = zone->points, j = 0; link != NULL; link = link->next, j++)
    {
      ...
      if (highlighted && (gint) j == priv->highlight_vertex)
...
hyscan_gtk_map_planner_save_parallel (HyScanGtkMapPlanner *planner)
{
  ...
  for (link = priv->parallel_tracks; link != NULL; link = link->next)
    {
      track = link->data;
      hyscan_object_model_add_object (HYSCAN_OBJECT_MODEL (priv->model), (HyScanObject *) track->object);
    }
}
