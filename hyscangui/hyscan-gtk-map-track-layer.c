...
struct _HyScanGtkMapTrackLayerPrivate
{
  ...
  HyScanBuffer               *cache_buffer;    /* Буфер для заголовка кэша. */
  HyScanBuffer               *tile_buffer;     /* Буфер для поверхности тайла. */
                                      ...
                                      HyScanGtkMapTrackLayerTile *tile)
{
  ...
  /* Копируем. */
  g_message (" cache_get %p", g_thread_self ());
                                      ...
                                      HyScanGtkMapTrackLayerTile *tile)
{
  ...
  g_message (" cache_set %p", g_thread_self ());
  ...
  tile_data = cairo_image_surface_get_data (tile->surface);
  hyscan_buffer_wrap_data (priv->tile_buffer, HYSCAN_DATA_BLOB, (gpointer) tile_data, size);
  ...
  hyscan_gtk_map_track_layer_update_cache_key (track_layer, tile);
  hyscan_cache_set2 (priv->cache, priv->cache_key, NULL, priv->cache_buffer, priv->tile_buffer);
}
