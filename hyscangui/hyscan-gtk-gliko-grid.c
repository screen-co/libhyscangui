...
G_DEFINE_TYPE_WITH_CODE( HyScanGtkGlikoGrid, hyscan_gtk_gliko_grid, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE( HYSCAN_TYPE_GTK_GLIKO_LAYER,
                                                layer_interface_init ) )
...
static void create_model( HyScanGtkGlikoGridPrivate *p )
{
...
static void hyscan_gtk_gliko_grid_init( HyScanGtkGlikoGrid *grid )
{
...
static void hyscan_gtk_gliko_grid_class_init( HyScanGtkGlikoGridClass *klass )
{
...
static float *get_pfloat( HyScanGtkGlikoGridPrivate *p, const int id )
{
  switch( id )
  {
  case P_CX:       return &p->cx;
  case P_CY:       return &p->cy;
  case P_SCALE:    return &p->scale;
  case P_COLOR_ALPHA: return &p->color[3];
  default:         break;
  }
  return NULL;
}

static float *get_prgb( HyScanGtkGlikoGridPrivate *p, const int id )
{
  switch( id )
  {
  case P_COLOR:     return p->color;
  default: break;
  }
  return NULL;
}
