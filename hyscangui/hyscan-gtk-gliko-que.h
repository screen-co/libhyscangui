...
static void initque( que_t *que, void *data, const int item_size, const int que_length )
{
  que->data = data;
  que->size = item_size;
...
static void enque( que_t *que, const void *srcv, const int items )
{
  int i, k, n, s;
  const char *src = srcv;
...
static int deque( que_t *que, void *dstv, const int items, int64_t count )
{
  int64_t d;
  int i, k, n, r, s;
  char *dst = dstv;
