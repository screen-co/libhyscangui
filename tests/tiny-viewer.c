...
main (int argc, char **argv)
{
  ...
  /* Парсим аргументы*/
  {
    ...
    context = g_option_context_new ("<db-uri>\n\nTiny data viewer.\nGenerates a full picture of a specified track and board.\n\
Currently it doesn't support any corrections.\n\
Please note:\n \
\t1. Starboard will NOT be mirrored.\n \
\t2. Most recent data is in the BOTTOM of the frame\n");
  ...
  switch (board)
    {
      case 2:
        ...
        filename = g_strdup_printf ("%s%s.%s.port.png", (filepath == NULL) ? "_" : filepath, project, track);
        break;
      case 1:
        lsrc = HYSCAN_LOCATION_SOURCE_SONAR_STARBOARD;
        filename = g_strdup_printf ("%s%s.%s.star.png", (filepath == NULL) ? "_" : filepath, project, track);
        break;
      case 0:
      default:
        lsrc = HYSCAN_LOCATION_SOURCE_ECHOSOUNDER;
        filename = g_strdup_printf ("%s%s.%s.echo.png", (filepath == NULL) ? "_" : filepath, project, track);
    }

  /* Теперь определяем, что у нас, собственно говоря, за задача. */
  tile = slant_no_correction (db, NULL, project, track, board, use_raw_src, &buffer, &buffer_size, FALSE);
                     ...
                     gboolean          show_dpt)
{
  ...
  if (!hyscan_raw_data_get_range	(dchannel, &lindex, &rindex))
    return tile;
  g_print ("Channel %s has %i lines of data.\n", channel_name, rindex - lindex + 1);
